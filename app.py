...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🚀 *Upload Bot*\n\n"
        f"Link gonder → Drive'a yukle\n\n"
        f"*Komutlar:*\n"
        f"/downloads - Aktif indirmeler\n"
        f"/settime {h} - Silme suresi\n"
        f"/status - Durum",
        parse_mode="Markdown"
    )
...
async def cmd_settime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        await update.message.reply_text(f"⏱️ Su an: {h} saat\n`/settime <saat>`", parse_mode="Markdown")
        return
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("c_"):
        ...
        if tid in active_downloads:
            ...
            await q.edit_message_text("⚠️ Bulunamadi")
...
async def process_download(task, msg, kb, delete_hours):
    ...
    try:
        ...
        if sz > 5 * 1024 * 1024 * 1024:
            await msg.edit_text(f"⚠️ Cok buyuk: {format_size(sz)} > 5GB", parse_mode="Markdown")
